...
> 주 배포 대상: Render.com
>
> 상태: **GitHub·Render 배포 재개 요청 접수. 예산 0원 유지.** GitHub 공개 소스 릴리스를 준비하며, Render 서비스 생성 전 무료 환경의 데이터 손실 조건과 개인용 인증 방식을 확인한다. 현재 저장소에는 아직 운영용 `Dockerfile`, 인증 게이트웨이, `render.yaml`이 없다.

## 1. 결론

사용자가 GitHub·Render 배포 재개를 요청했다. **GitHub 소스 공개는 진행하되, Render 무료 체험의 데이터 손실 조건 확인과 인증 경계 구현 전에는 서비스를 공개하지 않는다.** 아래의 유료 서비스·영구 디스크·운영 확장안은 향후 예산과 별도 승인이 있을 때의 검토안이며 실행 지시가 아니다.

이번 사전 점검에서는 공개 저장소 `kimdongup/shopitem-curator`의 `main`과 승인된 Render `My Workspace`를 확인했다. 해당 워크스페이스에는 기존 무료 Web Service들이 있으며, 아직 이 앱의 서비스는 없다. 기존 서비스는 변경하지 않는다. 월 사용량·결제 수단·초과 비용 제한은 서비스 목록만으로 확인할 수 없으므로 생성 전에 Dashboard에서 확인해야 한다.

**기능 차이:** 현재 Chrome 확장과 `/v1/browser-bridge/*`는 로컬 전용이다. 단순히 Render에 올려도 브라우저 직접 선택 기능이 원격으로 연결되지 않는다. 원격 연결을 지원하려면 HTTPS 서버 주소 선택, 프로젝트별 연결 인증과 Origin 검증을 먼저 구현·검증해야 한다. 로컬 브리지 제한을 해제하거나 그대로 외부에 프록시하는 것으로 대체하지 않는다.
